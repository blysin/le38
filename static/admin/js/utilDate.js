...
function getCurrentWeek(){ 
        ...
        var md;
        var sd;
        //处理周一的日期，转化成yyyy-mm-dd
        if(monday.getMonth()<9){
	        md = monday.getFullYear()+"-"+"0"+(monday.getMonth()+1);
	        
        }else{
        	md = monday.getFullYear()+"-"+(monday.getMonth()+1);
        }
        ...
        //处理周日的日期，转化成yyyy-mm-dd
        if(sunday.getMonth()<9){
	        sd = sunday.getFullYear()+"-"+"0"+(sunday.getMonth()+1);
	        
        }else{
        	sd = sunday.getFullYear()+"-"+(sunday.getMonth()+1);
        }
        ...
        //添加本周时间  
        startStop.push(md);//本周起始时间  
        //添加本周最后一天时间  
        startStop.push(sd);//本周终止时间  
        //返回  
        return startStop; 
    }; 
    /***
...
function getPriorMonthFirstDay(year,month){ 
    ...
    function getPreviousMonth(){ 
        ...
        var currentYear=currentDate.getFullYear(); 
        //获得上一个月的第一天  
        var priorMonthFirstDay=getPriorMonthFirstDay(currentYear,currentMonth,1); 
        //获得上一月的最后一天  
        ...
        //添加至数组  
        if(priorMonthFirstDay.getMonth()<9){
	        priorMonthFirstDay = priorMonthFirstDay.getFullYear()+"-"+"0"+(priorMonthFirstDay.getMonth()+1)+"-"+"0"+priorMonthFirstDay.getDate();
	        priorMonthLastDay = priorMonthLastDay.getFullYear()+"-"+"0"+(priorMonthLastDay.getMonth()+1)+"-"+priorMonthLastDay.getDate();
        }else{
        	priorMonthFirstDay = priorMonthFirstDay.getFullYear()+"-"+(priorMonthFirstDay.getMonth()+1)+"-"+"0"+priorMonthFirstDay.getDate();
...
function getPreviousWeek(){ 
        ...
        var sd;
        //处理周一的日期，转化成yyyy-mm-dd
        if(priorWeekFirstDay.getMonth()<9){
	        md = priorWeekFirstDay.getFullYear()+"-"+"0"+(priorWeekFirstDay.getMonth()+1);
	        
        }else{
        	md = priorWeekFirstDay.getFullYear()+"-"+(priorWeekFirstDay.getMonth()+1);
        }
        if(priorWeekFirstDay.getDate()<10){
        	md+="-"+"0"+priorWeekFirstDay.getDate();
        }else{
        	md+="-"+priorWeekFirstDay.getDate();
        }
        //处理周日的日期，转化成yyyy-mm-dd
        if(priorWeekLastDay.getMonth()<9){
	        sd = priorWeekLastDay.getFullYear()+"-"+"0"+(priorWeekLastDay.getMonth()+1);
	        
        }else{
        	sd = priorWeekLastDay.getFullYear()+"-"+(priorWeekLastDay.getMonth()+1);
        }
        if(priorWeekLastDay.getDate()<10){
        	sd+="-"+"0"+priorWeekLastDay.getDate();
        }else{
        	sd+="-"+priorWeekLastDay.getDate();
        }
        //添加至数组  
        startStop.push(md); 
        startStop.push(sd); 
