BUI.use(['bui/menu', 'bui/tab'], function (Menu, Tab) {
    ...
    function tabNav(moduleId, tabConfig, menuConfig, collapsed, homePage, topContent) {
        ...
        if(topContent){
            ...
            $(menuConfig.render).children(0).prepend(topContent);
            topContent.on("click", function (e) {
                var link = e.target;
                if(link.tagName == "A"){
                    _self.tab.addTab({id: link.id, title: link.title, href: link.href, closeable: true}, true);
                }
            })
        }
    }
    ...
    BUI.augment(mainPage, {
        ...
        _createModule: function (id) {
            ...
            var id = item.id,
                tabId = '#J_' + id + 'Tab',
                treeId = '#J_' + id + 'Tree';
            module = new tabNav(id, {render: tabId, height: getAutoHeight()}, {render: treeId, items: item.menu, height: getAutoHeight()}, item.collapsed, item.homePage, item.topContent);
            modules[id] = module;
            return module;
        },
        //隐藏列表
        ...
        _initNavItems: function () {
            ...
            var count = navItems.length,
                clientWidth = $(".dl-main-nav").width(),//获取菜单区域可用宽度
        ...
        _setLastItem: function (item) {
            ...
            if (!appendNode) {
                appendNode = $('<span class="nav-item-more ' + CLS_HIDE_CURRENT + '">&nbsp;&nbsp;</span>');
            }
        ...
        _setModuleSelected: function (index, sender) {
            ...
            if (currentModelIndex !== index) {
                ...
                //如果点击的是 门店webPos,弹出新页签
                if(moduleId == "webPosOrder"){
                    window.open("/admin/store/order/webpos/show");
                    return ;
                }
